...
function BookingRow({ booking }: { booking: Reservation }) {
  return <div className="flex min-w-0 flex-col gap-3 rounded-lg border border-slate-200 bg-white p-4 shadow-sm sm:flex-row sm:items-center sm:justify-between"><div className="min-w-0"><p className="truncate font-semibold text-slate-900">{booking.salleNom}</p><p className="mt-1 truncate text-sm text-slate-500">{booking.organisateur}{booking.description ? ` · ${booking.description}` : ""}</p></div><div className="flex flex-wrap items-center gap-x-5 gap-y-2 text-sm text-slate-600"><span className="flex items-center gap-2 whitespace-nowrap"><Clock3 size={16} className="shrink-0 text-amber-500" />{timeLabel(booking.dateDebut)} – {timeLabel(booking.dateFin)}</span><span className="flex items-center gap-1.5 whitespace-nowrap"><Users size={16} />{booking.nombreParticipants ?? "—"}</span></div></div>;
}
