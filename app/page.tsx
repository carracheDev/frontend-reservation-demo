...
export default function Home() {
  ...
  const now = new Date();
  return <main className="min-h-screen bg-slate-50"><nav className="bg-slate-900 text-white"><div className="mx-auto flex max-w-7xl items-center justify-between px-5 py-4 lg:px-8"><Link href="/" className="flex items-center gap-3 text-lg font-bold tracking-tight"><span className="flex h-9 w-9 items-center justify-center rounded-lg bg-amber-400 text-slate-950"><CalendarDays size={19} /></span>RoomBook</Link><div className="flex items-center gap-4 text-sm text-slate-300"><span className="hidden sm:block">Planning d&apos;équipe</span><span className="h-2 w-2 rounded-full bg-emerald-400" /> En ligne</div></div></nav>
    <section className="border-b border-slate-200 bg-white"><div className="mx-auto max-w-7xl px-5 pb-10 pt-12 lg:px-8"><p className="text-sm font-semibold uppercase tracking-[0.18em] text-amber-600">Vue d&apos;ensemble</p><div className="mt-3 flex flex-col justify-between gap-5 md:flex-row md:items-end"><div><h1 className="text-4xl font-bold tracking-[-0.04em] text-slate-900">Votre planning, en un coup d&apos;œil.</h1><p className="mt-3 text-slate-500">Gérez les espaces de travail de votre équipe sans friction.</p></div><div className="text-sm font-medium text-slate-500">{now.toLocaleDateString("fr-FR", { weekday: "long", day: "numeric", month: "long" })}</div></div>
      <div className="mt-10 grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><div className="stat"><Building2 className="text-amber-500" size={19} /><div><p className="text-2xl font-bold text-slate-900">{loading ? "—" : stats?.nombreTotalSalles ?? 0}</p><p className="text-xs font-medium uppercase tracking-wide text-slate-500">Salles actives</p></div></div><div className="stat"><CalendarDays className="text-amber-500" size={19} /><div><p className="text-2xl font-bold text-slate-900">{loading ? "—" : stats?.nombreReservationsAujourdhui ?? 0}</p><p className="text-xs font-medium uppercase tracking-wide text-slate-500">Réservations aujourd&apos;hui</p></div></div><div className="stat"><CalendarDays className="text-amber-500" size={19} /><div><p className="text-2xl font-bold text-slate-900">{loading ? "—" : stats?.nombreReservationsAVenir ?? 0}</p><p className="text-xs font-medium uppercase tracking-wide text-slate-500">Réservations à venir</p></div></div><div className="stat"><div className="relative h-10 w-10 rounded-full border-4 border-slate-200"><span className="absolute inset-[-4px] rounded-full border-4 border-amber-400" style={{ clipPath: `inset(${100 - (stats?.tauxOccupationJour ?? 0)}% 0 0 0)` }} /></div><div><p className="text-2xl font-bold text-slate-900">{loading ? "—" : `${stats?.tauxOccupationJour ?? 0}%`}</p><p className="text-xs font-medium uppercase tracking-wide text-slate-500">Occupation du jour</p></div></div></div>
    </div></section>
