...
export default function SalleDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!salle) return <main className="mx-auto max-w-7xl px-5 py-10"><Link href="/" className="text-sm text-amber-600">Retour au dashboard</Link><div className="mt-16 h-48 animate-pulse rounded-lg bg-slate-200" /></main>;
  ...
  return <main className="min-h-screen bg-slate-50">{notice?.kind === "success" && <div className="toast fixed right-5 top-5 z-20 flex items-center gap-3 rounded-lg border border-emerald-200 bg-white px-4 py-3 text-sm font-semibold text-emerald-700 shadow-lg"><CheckCircle2 size={18} /> {notice.text}</div>}<nav className="bg-slate-900 text-white"><div className="mx-auto flex max-w-7xl items-center justify-between px-5 py-4 lg:px-8"><Link href="/" className="flex items-center gap-3 text-lg font-bold tracking-tight"><span className="flex h-9 w-9 items-center justify-center rounded-lg bg-amber-400 text-slate-950"><CalendarDays size={19} /></span>RoomBook</Link><Link href="/" className="inline-flex items-center gap-2 text-sm text-slate-300 transition hover:text-white"><ArrowLeft size={16} /> Dashboard</Link></div></nav>
    <section className="mx-auto max-w-7xl px-5 pb-16 pt-10 lg:px-8"><div className="flex flex-col justify-between gap-5 border-b border-slate-200 pb-8 md:flex-row md:items-end"><div><div className="flex items-center gap-3"><span className="rounded-md bg-amber-100 px-2.5 py-1 text-xs font-bold uppercase tracking-wide text-amber-800">{salle.type}</span><span className="flex items-center gap-1.5 text-sm text-slate-500">{salle.type === "CONFERENCE" ? <Presentation size={15} /> : <Users size={15} />} {salle.capacite} places</span></div><h1 className="mt-4 text-4xl font-bold tracking-[-0.04em] text-slate-900">{salle.nom}</h1><p className="mt-2 text-slate-500">Planning et réservation de la salle.</p></div><label className="text-sm font-medium text-slate-600">Jour du planning<input type="date" value={selectedDay} onChange={(event) => setSelectedDay(event.target.value)} className="field mt-2 w-auto" /></label></div>
